...
This repository is private and preparing a v0.7.0 release candidate. It has no published GitHub
Release or PyPI release. Its intended Python distribution is `phamviet-local-rag-mcp`; the existing
PyPI project named `local-rag-mcp` is unrelated. Do not install, report vulnerabilities to, or infer
support status from that other project.
...
After the repository is deliberately published, use GitHub's private **Report a vulnerability**
workflow only if it has been enabled for this repository. Otherwise contact the maintainer privately
before disclosure. Do not open a public issue containing credentials, private document text, source
paths, database contents, or an unpatched exploit.
